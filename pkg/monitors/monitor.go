...
var AvailableMetrics = []string{"la", "mem", "cpu"}
...
type MetricsPresent struct {
	la  bool
	cpu bool
	mem bool
}

func GetOsMonitor(log *logrus.Logger, goos string, metrics *MetricsPresent) (*OsMonitor, error) {
...
func (om *OsMonitor) Run(ctx context.Context) {
	...
	for {
		...
		case <-scheduler.C:
			...
			state := om.stateCollector.GetCurrentState(ctx, om.metrics)
			om.mx.Lock()
			...
			for k := range om.avgRequired {
				...
				case k > len(om.states):
					continue
				case k == len(om.states):
					om.averages[k] = calcAvg(om.states, om.metrics)
				default:
					if _, ok := om.averages[k]; !ok {
						om.averages[k] = calcAvg(om.states, om.metrics)
					} else {
						startIdx := len(om.states) - k
						om.averages[k] = calcIncrementAvg(om.states[startIdx-1:], om.averages[k], om.metrics)
					}
				}
			}
